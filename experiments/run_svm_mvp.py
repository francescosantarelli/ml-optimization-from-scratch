...
from src.svm_mvp import mvp_decomposition
...
def main():
    ...
    start = time.time()
    alpha, b, sv_mask, obj, info = mvp_decomposition(
    ...
    print("SVM — custom Most Violating Pair decomposition")
    print(f"status: {info['status']}")
    print(f"pair updates: {info['iterations']}")
    print(f"dual objective: {obj:.6f}")
    print(f"final KKT gap: {info['final_gap']:.6e}")
